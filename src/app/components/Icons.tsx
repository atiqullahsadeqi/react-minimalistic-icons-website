...
const IconItem = memo(
  ({
    ...
    return (
      ...
      >
        <div className="relative bg-background  h-20 w-20 sm:h-24 sm:w-24 md:h-28 md:w-28 lg:h-32 lg:w-32 xl:h-[120px] xl:w-[120px] border border-muted rounded-xl flex items-center justify-center duration-300">
          <IconComponent
            size={16}
            className="sm:w-5 sm:h-5 md:w-6 md:h-6 lg:w-7 lg:h-7 xl:w-6 xl:h-6 text-foreground"
          />
        ...
        <p className="text-muted-foreground mt-2 sm:mt-3 text-xs sm:text-xs text-center leading-tight">
          {icon.name
            .replace(/([A-Z])/g, "-$1")
            .toLowerCase()
            .replace(/^-/, "")}
...
const Icons = memo(({ icons }: IconGridProps) => {
  ...
  return (
    <div className="w-full flex justify-center px-4 sm:px-6 lg:px-8">
      <div className="w-full max-w-7xl grid grid-cols-3 sm:grid-cols-4 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-3 sm:gap-4 lg:gap-5">
        {icons.map((icon) => (
          <IconItem
            key={icon.name}
            icon={icon}
